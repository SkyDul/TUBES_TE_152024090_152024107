...
<head>
    ...
    <title>Antrean Cash + Deteksi - RipaNet POS</title>
...
<body>
    <div class="container admin-shell">
        <header class="admin-topbar">
            <a class="brand" href="index.php">
                ...
                <span class="brand__meta">
                    <strong>Antrean Cash</strong>
                    <span>Scan uang dulu, approval admin setelah verdict genuine</span>
                </span>
            </a>
            <nav class="admin-topbar__links">
                <a href="index.php">Dashboard</a>
                <a href="pos.php">POS Kasir</a>
                <a href="cash-orders.php" class="active">Antrean Cash</a>
        ...
        <section class="admin-kpis">
            <article class="kpi-card">
                <div class="kpi-card__label">Pending Queue</div>
            ...
            <article class="kpi-card">
                <div class="kpi-card__label">Cash Lunas Hari Ini</div>
            ...
            <article class="kpi-card">
                <div class="kpi-card__label">Mode Detector</div>
                <div class="kpi-card__value" style="font-size: 1.2rem;"><?= env('CASH_DETECTOR_URL') ? 'REMOTE' : 'DUMMY' ?></div>
                <div class="kpi-card__hint">Siap diganti ke Python EC2</div>
            </article>
            <article class="kpi-card">
                <div class="kpi-card__label">Kasir Aktif</div>
                <div class="kpi-card__value" style="font-size: 1.2rem;"><?= htmlspecialchars($adminUser) ?></div>
                <div class="kpi-card__hint">Gunakan scan sebelum approval</div>
            </article>
        </section>
        ...
        <section class="dashboard-grid">
            <section class="panel">
                <div class="panel-head">
                    <div>
                        <h2>Queue Cash Pending</h2>
                        <p>Setiap order wajib scan uang dulu. Tombol approval aktif hanya jika verdict genuine.</p>
                    </div>
                    <a class="btn btn-secondary btn-sm" href="cash-orders.php">Refresh</a>
                </div>
                        ...
                        <?php foreach ($pendingOrders as $order): ?>
                            ...
                            <article class="queue-item" data-order-id="<?= htmlspecialchars($order['order_id']) ?>" data-verdict="<?= htmlspecialchars($verdict, ENT_QUOTES, 'UTF-8') ?>">
                                ...
                                <div class="queue-actions">
                                    <button type="button" class="btn btn-secondary btn-sm detect-cash-btn" data-order-id="<?= htmlspecialchars($order['order_id']) ?>">Scan Uang</button>
                                    <button type="button" class="btn btn-primary btn-sm approve-btn" data-order-id="<?= htmlspecialchars($order['order_id']) ?>" <?= $verdict === 'genuine' ? '' : 'disabled' ?>>Approve & Lunasi</button>
            ...
            <section class="panel">
                <div class="panel-head">
                    <div>
                        <h3>Output Deteksi / Approval</h3>
                        <p>Status proses untuk order yang sedang dikerjakan kasir.</p>
                    </div>
                </div>
                <div class="terminal-output">
                    <div class="terminal-output__label">Cash Security Console</div>
                ...
                <div class="panel-head" style="margin-top: 20px;">
                    <div>
                        <h3>Riwayat Cash Hari Ini</h3>
                        <p>Settlement terbaru setelah approval.</p>
                    </div>
                </div>
    ...
    function wireQueueItem(item) {
        ...
        detectBtn.addEventListener('click', async () => {
            ...
            if (!userCash) {
                const inputStr = prompt('Masukkan nominal uang yang dibayar pelanggan (misal: 100000):');
                if (!inputStr) return;
        ...
        approveBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                alert(`Transaksi Berhasil Lunas!\nVoucher: ${data.voucher_username}`);
            } catch (error) {
